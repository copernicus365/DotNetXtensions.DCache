...
/// dependency on the internally used <see cref="CacheDictionary{TKey, TValue}"/>.
/// </summary>
...
public class DCache<T, TId> : IDCache<T, TId>
{
	...
	public DCacheGetSrc LastCacheSrc { get; private set; }
	...
	public async Task<T> GetAsync(TId id, bool skipMemCache = false, CancellationToken token = default)
	{
		string key = GetCacheKey(id);

		if(!skipMemCache && MemoryCacheIsEnabled && MemCacheDict.TryGetValue(key, out CacheData val)) {
			LastCacheSrc = DCacheGetSrc.MemoryCache;
			return val.Item;
		}

		byte[] data = await _dcache.GetAsync(key, token);

		if(data == null || data.Length == 0) {
			LastCacheSrc = DCacheGetSrc.None;
			return default;
		}

		LastCacheSrc = DCacheGetSrc.DistributedCache;
	...
	public async Task RefreshAsync(TId id, CancellationToken token = default)
	{
		string key = GetCacheKey(id);

		await _dcache.RefreshAsync(key, token);

		if(MemoryCacheIsEnabled && MemCacheDict.TryGetValue(key, out CacheData dd))
			MemCacheDict[key] = new CacheData(DateTimeOffset.UtcNow, dd.Item);
	}

	public async Task RemoveAsync(TId id, CancellationToken token = default)
	{
		string key = GetCacheKey(id);

		MemCacheDict?.Remove(key);
	...
	public async Task SetAsync(TId id, T item, DistributedCacheEntryOptions options = null, CancellationToken token = default)
	{
